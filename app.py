...
def query_train():
	...
	if request.method == 'POST':
		...
		if date_ == '!':
			flash('Invalid input: "'+ date +'". (A valid date should like "5-12".')
			return redirect(url_for('query_train'))
		...
		###############################	
		if trains_str == '-1':
			flash('Train "'+ train_id +'" on ' + date_ +' not found.')
			return redirect(url_for('query_train'))
	...
	return render_template('query_train.html', trains=trains, train_id=train_id_, train_type=train_type, q_train_display=q_train_display)
